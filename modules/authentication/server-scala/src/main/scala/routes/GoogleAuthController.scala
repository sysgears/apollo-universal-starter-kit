  ...
  val routes: Route =
    (path("auth" / "google") & get) {
      ...
      parameters('state.?, 'code) { (state, code) =>
        onComplete {
          for {
            googleAuthInfo <- externalApiService
              .getUserInfo[GoogleOauth2Response](
                code,
                "https://content.googleapis.com/oauth2/v2/userinfo",
                oauth2Service)
            user <- googleAuthRepository
              .findOne(googleAuthInfo.id)
              .run
              .flatMap {
                case Some(gUser) =>
                  userRepository
                    .findOne(gUser.userId)
                ...
                case None =>
                  for {
                    user <- userRepository
                      .save(
                        User(username = googleAuthInfo.name,
                             ...
                             password =
                               BCrypt.hashpw(googleAuthInfo.id, BCrypt.gensalt),
                             role = "user",
                             isActive = true)
                      )
                      .run
                    _ <- googleAuthRepository
                      .save(
                        GoogleAuth(Some(googleAuthInfo.id),
                                   googleAuthInfo.name,
                                   user.id.get))
                      .run
                  } yield user
