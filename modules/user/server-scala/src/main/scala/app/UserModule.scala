...
class UserModule extends ServerModule[UserContext, SchemaInitializer[_]] {
  ...
  lazy val userSchemaInitializer: UserSchemaInitializer =
    inject[UserSchemaInitializer]
  lazy val userProfileSchemaInitializer: UserProfileSchemaInitializer =
    inject[UserProfileSchemaInitializer]
  ...
  override lazy val queries: mutable.HashSet[Field[UserContext, Unit]] =
    mutable.HashSet(userSchema.queries: _*)
