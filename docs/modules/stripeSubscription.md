...
4. Add Stripe secret endpoint key from your Stripe webhook ([webhook signatures]) in `packages/server/.env` to prevent 
fraudulent webhooks from being processed.
5. Run `yarn stripe:setup` from the root directory to create a subscription plan and product using Stripe API. The
 default plan and product are 
...
By default, `webhookUrl` is set to `/stripe/webhook` in `config/stripe/subscription.js`; therefore, in the Stripe 
dashboard you need to set the webhook endpoint to `https://your-website-name.com/stripe/webhook`.  
2. Add your **live** publishable key, secret key and secret endpoint keys from Stripe to your deployment configuration
...
`STRIPE_ENDPOINT_SECRET` - secret endpoint key to prevent fraudulent webhooks from being processed ([webhook signatures]).

**NOTE**: `yarn stripe:setup` yarn command will run automatically by [heroku] to create a subscription plan and product using 
Stripe API. The default plan and product are configured in the `config/stripe/subscription.js` file.
3. Deploy your app.
