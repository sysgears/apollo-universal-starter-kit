...
function startServerWebpack(node) {
  try {
    const reporter = (...args) => webpackReporter(node.config.output.path, logBack, ...args);

    if (__DEV__) {
      _.each(node.config.entry, entry => {
        if (__WINDOWS__) {
          entry.push(require.resolve('webpack/hot/poll.js') + '?1000');
        } else {
          entry.push(require.resolve('webpack/hot/signal.js'));
        }
      });
      node.config.plugins.push(new webpack.HotModuleReplacementPlugin(),
        new webpack.NoEmitOnErrorsPlugin());
    }

    const compiler = webpack(node.config);
...
const node = { config, dll, platform };
