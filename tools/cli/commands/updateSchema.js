...
`;
      ...
      return ctx.${schema.name}.getByIds(sources.map(({ id }) => id), '${remoteField}', ctx.${value.type[0].name}, info);
    }),
`;
        }
      }
...
`;
      ...
      logger.info(chalk.green(`✔ Resolver in ${destinationPath}${resolverFile} successfully updated!`));
            */
      logger.info(chalk.green(`✔ Module ${moduleName} successfully updated!`));
    } else {
