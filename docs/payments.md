...
the file `config/subscription.js`.
...
| webhookUrl     | String  | Stores a URL for your webhook that's used by Stripe to ensure secure connection to Stripe |
...
Your `product` object may look like this:
...
In order to run the subscription module, you must provide at least a name and type for your product.

### Subscription Plan

The `plan` object allows you to describe the plans you want your application to provide. Consult the table below to know
how the object is configured:

| `plan`         | Value   | Purpose |
| -------------- | ------- | ------- |
| id             | string  | The ID for the subscription plan. Can be set any string description. Defaults to `basic` |
| nickname       | string  | The name of the subscription plan. Rename to "title"  |
| amount         | Number  | The amount in cents. Floating point numbers are accepted                            |
| interval       | String  | String description for the subscription period. Rename to period. Use `week`, `month`, `year` |
| currency       | String  | Lowercase currency value. Defaults to USD |
...
5. Add stripe signing secret key from your stripe webhook in your `.env`. 
5. Run `yarn stripe:setup` from the directory `packages/server/` to create a subscription plan using Stripe API.
