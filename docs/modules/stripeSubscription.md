...
```
3. Replace the `process.env.STRIPE_PUBLIC_KEY` property with your Stripe publishable key for **development mode** in 
the `config/stripe/subscription.js` configuration file:
...
can add stripe-local properties to the object attribute passed to `stripeLocal()`:
...
To use the subscription module, you can add Stripe keys into the `packages/server/.env` file and configure the module in 
the `config/stripe/subscription.js` file.
...
3. Add your live secret key from Stripe in `packages/server/.env`.
...
5. Run `yarn stripe:setup` from the root directory of Apollo Starter kit to create a subscription plan and product 
using the Stripe API. The default plan and product are configured in the `config/stripe/subscription.js` file.
...
`STRIPE_ENDPOINT_SECRET` - secret endpoint key to prevent fraudulent webhooks from being processed 
([webhook signatures]).

**NOTE**: The command `yarn stripe:setup` will run automatically by [Heroku] to create a subscription plan and product 
using the Stripe API. The default plan and product are configured in the `config/stripe/subscription.js` file.
3. Deploy your app.
