        ...
        onComplete {
          for {
            githubAuthInfo <- externalApiService
              .getUserInfo[GithubOauth2Response](code,
                                                 "https://api.github.com/user",
                                                 oauth2Service)
            user <- githubAuthRepository
              .findOne(githubAuthInfo.id)
              .run
              .flatMap {
                case Some(ghUser) =>
                  userRepository
                    .findOne(ghUser.userId)
                ...
                case None =>
                  for {
                      ...
                      .save(
                        User(
                          ...
                          password = BCrypt.hashpw(
                            githubAuthInfo.id.toString + githubAuthInfo.name,
                            BCrypt.gensalt),
                          role = "user",
                          isActive = true
                        )
                    ...
                    _ <- githubAuthRepository
                      .save(
                        GithubAuth(Some(githubAuthInfo.id),
                                   githubAuthInfo.name,
                                   user.id.get))
                      .run
                  } yield user
