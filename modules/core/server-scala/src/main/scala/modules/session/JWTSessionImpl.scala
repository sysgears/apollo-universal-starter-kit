...
class JWTSessionImpl @Inject()(implicit val executionContext: ExecutionContext)
  ...
  override implicit val encoder: JwtSessionEncoder[SessionData] =
    new JwtSessionEncoder[SessionData]
  override implicit val manager: SessionManager[SessionData] =
    new SessionManager(sessionConfig)
  override implicit val refreshTokenStorage
    : InMemoryRefreshTokenStorage[SessionData] = (msg: String) => log.info(msg)

  def withNew(sessionData: SessionData): Directive0 =
    setSession(refreshable, usingCookies, sessionData)

  def withOptional: Directive1[Option[SessionData]] =
    optionalSession(refreshable, usingCookies)

  def withInvalidating: Directive0 =
    invalidateSession(refreshable, usingCookies)

  def withChanges(requestSession: Option[SessionData],
                  newSession: Option[SessionData]): Directive0 =
    newSession match {
