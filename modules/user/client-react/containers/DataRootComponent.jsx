...
const DataRootComponent = props => {
  const [isReady, setIsReady] = useState(settings.auth.session.enabled);

  useEffect(() => {
    (async () => {
      if (!isReady && (await getItem('refreshToken'))) {
        const { client } = props;
        let result;
        try {
  ...
  return isReady ? props.children : <Loading />;
};
