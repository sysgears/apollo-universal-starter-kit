...
case class GithubOauth2Response(id: Int,
                                email: String,
                                name: String)
...
object GithubOauth2Response extends DefaultJsonProtocol {
  ...
  implicit object GithubOauth2ResponseFormat extends RootJsonFormat[GithubOauth2Response] {
    def write(response: GithubOauth2Response) = JsArray(JsNumber(response.id), JsString(response.email), JsString(response.name))
