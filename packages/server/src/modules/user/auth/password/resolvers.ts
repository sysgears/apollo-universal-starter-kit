...
const validateUserPassword = async (user: any, password: string, t: (s: string) => string) => {
  ...
  Mutation: {
    ...
    async register(obj, { input }, { mailer, User, req }) {
      try {
        ...
        const emailExists: any = await User.getUserByEmail(input.email);
        if (emailExists) {
        ...
        if (!emailExists) {
          ...
          // XXX: why `role: undefined`? User.register is inside of js file and it has no optional properties
          [userId] = await User.register({ role: undefined, ...input, isActive });
    ...
    async resetPassword(obj, { input }, { req: { t }, User }) {
      try {
        const e = new FieldError();
        ...
        const { email, password }: any = jwt.verify(token, settings.user.secret);
